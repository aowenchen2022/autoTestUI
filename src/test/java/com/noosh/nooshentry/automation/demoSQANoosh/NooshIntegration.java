...
public class NooshIntegration extends BaseSeleniumTest {
	...
	public static void setParameters(ITestContext context)
		...
		if (domain.trim().equals("spd")) {
			...
			baseUrlSmoke = "http://nooshselenium2." + domain
					+ ".noosh.com/service/login";
			smokeURL = "https://nooshselenium." + domain
					+ ".noosh.com/service/login";
			clientURL = "https://nooshselenium." + domain
					+ ".noosh.com/autoclient/main";
			loginUrl = "https://selenium263945." + domain
					+ ".noosh.com/service/login";
		}
	}
	...
	@Test(description = "testStapleURL")
	@Parameters({ "spEmailSmoke", "passwordSP" })
	public void testStapleURL() throws InterruptedException {
		log.info("Start: testStapleURL");
		Thread.sleep(1500);
		String stapleURL = "https://sourceexpress.scd.noosh.com/naso/salesforceAutoLogin/?AMFirstName=Rocco&AMLastName=Danella&AMTitle=Sales+Operations+Analyst&AMEmail=phillide&AMPhone=(508) 253-4364&CustName=Hot+Shot&ContactFirstName=Victor&ContactLastName=Lalo&AddressLine1=1407 Broadway&City=New York&State=New York&Country=&ZipCode=&Phone=(212) 921-1111 x13&Email=victor@hotshotny.com&DSM=Christopher Moran&RSM=null&District=null&Region=null&DeliveryDt=11%2F08%2F18&Product=Banner&Stage=First Meeting Completed&Amount=500.00&Description=AJ+testing+123+&Rewards=2041533627&Industry=Clothing and Apparel Manufacturing&LeadSource=Other%26Null&MasterAccount=&MasterDiv=&ShipToNumber=null&StoreNo=0008&UserId=00538000003Ok6gAAC&timestap=2017-10-20 13:04:54&integration_key=cLAXKNxGjGGn3fnKxzOsEBuA8M9%2F1vFh";
		driver.get(stapleURL);
		driver.manage().window().maximize();
		Thread.sleep(1500);
		driver.findElement(By.id("dashItemList")).findElement(By.tagName("div")).click();
		Thread.sleep(1500);
		String actualURL = driver.getCurrentUrl();
		Assert.assertEquals(actualURL, "https://sourceexpress.scd.noosh.com/site/naso/home");
		WebElement label_team =  driver.findElement(By.id("topMenuTeam"));
		WebElement container_projlist = driver.findElement(By.id("projectListControllerContainer"));
		
		String css_display= label_team.getCssValue("display");
		String css_margin = container_projlist.getCssValue("margin-top");
		Assert.assertEquals(css_display,"list-item");
		Assert.assertEquals(css_margin,"8px");
		System.out.println("css_display is:"+css_display );
		System.out.println("css_margin is:"+css_margin );
		log.info("End: test Staple Login");
		log.info("--------------------------------------------");
	}

	@Test(description = "SP login")
	@Parameters({ "spEmailSmoke", "passwordSP" })
	public void testSPLogin(String spEmailSmoke, String passwordSP)
	...
	public void testSPInputProjectName(String productNameIntegration,
	...
	public void testSPSelectWorkspace(String clientOrWorkspace)
			throws InterruptedException {
	...
	public void testSPSelectClient(String clientUser1)
			throws InterruptedException {
	...
	@Parameters({ "clientOrWorkspace" })
	public void testSPSelectSmartForm(String clientOrWorkspace)
			throws InterruptedException {
		log.info("Start: testSPSelectSmartForm");
		try {
			AssertJUnit.assertTrue(Page
					.isTextPresent(clientOrWorkspace, driver));
		} catch (Throwable e) {
			System.out.println("Client or Workspace field is missing!\n");
			log.info("Client or Workspace field is missing!");
		}
		WebElement startElement = driver.findElement(By
				.id("createProjectProductList"));
	...
	@Parameters({ "projNameSP", "descriptionNameSP", "sku", "referenceNumber",
			"quantSP", "quantSP2", "quantSP3", "specDescrSP",
			"fileForProject11" })
	public void testSPcreateProjectWizard(String projNameSP,
			String descriptionNameSP, String sku, String referenceNumber,
			String quantSP, String quantSP2, String quantSP3,
			String specDescrSP, String fileForProject11)
			throws InterruptedException, AWTException {
		log.info("Start: testSPcreateProjectWizard");
		CommonUtils commonUtils = new CommonUtils(driver);
		commonUtils.testSPcreateProject(projNameSP, descriptionNameSP, sku,
				referenceNumber, quantSP, quantSP2, quantSP3, specDescrSP,
				fileForProject11);
	...
	public void testGotoNEPage(String siteListPageTitle, String profileTitlePro)
			throws InterruptedException {
		...
		// WebElement img =
		// driver.findElement(By.className("glb-dropdown-control")).findElement(By.tagName("img"));
		// img.click();
	...
	public void testEditSFAgain() throws InterruptedException {
		log.info("Start: testEditSF");
		driver.findElement(By.id("cFTlWiXf"))
				.findElement(By.className("com-inlineEditor-value"))
				.sendKeys("1234567");
		Thread.sleep(2000);
		log.info("End: testEditSF");
		log.info("--------------------------------------------");
	...
	public void testOpenSFTab(String updatedSFName) throws InterruptedException {
		...
		Thread.sleep(3000);
		Assert.assertTrue(true, "Updated smart form name " + updatedSFName
				+ " updated in NE does NOT show in NGE.");
	...
	public void testOpenSFTabAgain(String specNameUpdate, String updatedSFName)
			throws InterruptedException {
		testOpenSFTab(updatedSFName);
		Assert.assertTrue(true, "The updated spec " + specNameUpdate
				+ " does NOT show in NGE.");
	}
	...
	@Parameters({ "postContent" })
	public void testNEUpdatedMsgInNGE(String postContent)
			throws InterruptedException {
		...
		Thread.sleep(3000);
		commonUtils
				.testOpenProjectSFTabInWS3(projectName + " edit", "Messages");
		Thread.sleep(6000);
		Assert.assertTrue(true, "Msg " + postContent
				+ " posted in NE does NOT show in NGE.");
	...
	public void testUploadFileNE(String fileForProject3)
			throws InterruptedException, AWTException {
	...
	/*
	 * @Test(description="In NE, select update sharing") public void
	 * testUpdateSharing() throws InterruptedException {
	 * log.info("Start: testUpdateSharing"); CommonUtils commonUtils = new
	 * CommonUtils(driver); commonUtils.testUpdateShare(); Thread.sleep(1500);
	...
	@Parameters({ "fileForProject3" })
	public void testNEUpdatedFileNGE(String fileForProject3)
			throws InterruptedException {
		...
		String imgName = driver.findElement(By.className("uploaded-file-name"))
				.getAttribute("title");
